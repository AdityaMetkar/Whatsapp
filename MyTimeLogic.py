...
class MyTimeLogicAdapter(LogicAdapter):
    """
    ...
    def process(self, statement, additional_response_selection_parameters=None):
        #now = datetime.now()
        AK = pytz.timezone("Asia/Kolkata") 
        timeInAK = datetime.now(AK)
        currentTimeInAK = timeInAK.strftime("%H:%M:%S")

        time_features = self.time_question_features(statement.text.lower())
        confidence = self.classifier.classify(time_features)
        response = Statement(text='The current time is ' + currentTimeInAK)
